...
fun CheckInScreen(
    ...
    when (viewModel.currentStep) {
        1 -> {
            // Kita belum buat UI MoodSelectionStep secara detail, sementara ke Step 2 dulu
            // MoodSelectionStep(onMoodSelected = { viewModel.currentStep = 2 })
            // (Simulasi sementara untuk memajukan step)
            QuestionsStep(
                viewModel = viewModel,
                onBack = onBack,
                onNext = { viewModel.currentStep = 2 } // Ke layar pertanyaan
            )
        }
        2 -> {
            QuestionsStep(
                ...
                Box(modifier = Modifier.fillMaxWidth()) {
                    IconButton(
                        ...
                        modifier = Modifier
                            ...
                            .size(40.dp)
                            .background(Color(0xFF2A2567), shape = androidx.compose.foundation.shape.CircleShape)
                    ) {
                        Icon(
                    ...
                    Column(
                        ...
                        // Emoji menyesuaikan pilihan user sebelumnya (Contoh: emoji5)
                        Image(
                    ...
                    when (question.domain) {
                                    ...
                                    question.options.forEach { option ->
                                        Row(
                                            ...
                                            RadioButton(
                                                ...
                                                onClick = {
                                                    viewModel.userAnswers = viewModel.userAnswers.toMutableMap().apply {
                                                        put(question.id, option.weight)
                                                    }
                                                },
                                                colors = RadioButtonDefaults.colors(selectedColor = blueOutline)
                        ...
                        Domain.FISIK -> {
                            ...
                            FlowRow(
                                ...
                                question.options.forEach { option ->
                                    val isSelected = viewModel.userAnswers[question.id] == option.weight
                                    FilterChip(
                                        selected = isSelected,
                                        onClick = {
                                            viewModel.userAnswers = viewModel.userAnswers.toMutableMap().apply {
                                        ...
                                        colors = FilterChipDefaults.filterChipColors(
                                            containerColor = Color.White,
                                            selectedContainerColor = Color(0xFF9383CC).copy(alpha = 0.3f),
